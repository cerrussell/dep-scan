...
def vdr_to_csaf(res):
    ...
    """
    cve = res["id"]
    acknowledgements = get_acknowledgements(res["source"])
    pkgs = [i["ref"] for i in res["affects"]]
    cwe, notes = parse_cwe(res["cwes"])
    product_status = get_product_status(res["affects"])
    cvss_v3 = parse_cvss(res["properties"])
    description = (
        res["description"]
        .replace("\n", " ")
        .replace("\t", " ")
    ...
    ids, references = format_references(res["advisories"])
    orig_date = res["published"] or None
    update_date = res["updated"] or None
...
def parse_cvss(props):
    ...
    version = re.findall(r"3.0|3.1", cvss_v3.get("cvssVectorString"))
    # baseScore, vectorString, and version are required for a valid score
    if not cvss_v3 or not version or not cvss_v3.get("cvssBaseScore"):
        return None
    version = version[0]
    return {
        "baseScore": cvss_v3["cvssBaseScore"],
        "attackVector": cvss_v3["cvssAttackVector"],
        "privilegesRequired": cvss_v3["cvssPrivilegesRequired"],
        "userInteraction": cvss_v3["cvssUserInteraction"],
        "scope": cvss_v3["cvssScope"],
        "baseSeverity": cvss_v3["cvssBaseSeverity"],
        "version": version,
        "vectorString": cvss_v3["cvssVectorString"],
    }
